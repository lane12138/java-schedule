package com.choupangxia.schedule.demo.task.other;
...
public class HuiYi2Task {
    ...
    public   void qiangHuiyishi(){
        ...
        String userId = LeiConfig.userId;
        ...
        String dayString = DateUtil.format(DateUtil.offsetDay(dateTime, LeiConfig.day), "yyyy-MM-dd");




        //判断当前时间是否是工作日
        if(!HolidayService.isZhouwu2(DateTime.of(dayString,"yyyy-MM-dd").getTime()) || HolidayService.isXiuxi(dayString)){
        ...
        for (List<String> time : LeiConfig.times) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            log.info("================开始会议室抢占2222 ===============  ");
            log.info("================"+start.get("dateTime")+"-"+end.get("dateTime")+" ===============  ");

            //2.创建日程
            String riChengId = huiTools.createRiCheng2(token, userId, start, end);

            //3.添加会议室
            boolean  flag= huiTools.addHuiyishi(token, userId, riChengId, LeiConfig.room.get(i));


            if(!flag){//如果添加会议室失败，尝试添加其他会议室
                for (String room : LeiConfig.rooms) {
